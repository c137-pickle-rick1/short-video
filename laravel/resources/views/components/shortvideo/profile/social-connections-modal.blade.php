            ...
            >
              @if(!empty($tab['items']))
                <div class="divide-y divide-gray-200">
                  @foreach($tab['items'] as $item)
                    <x-shortvideo.profile.social-connection-item
                      :item="$item"
                      :login-url="$loginUrl"
                    />
                  @endforeach
              ...
              @else
                <div class="flex h-full items-center justify-center py-4">
                  <x-ui.empty-state
                    :icon-class="$tab['emptyState']['iconClass'] ?? 'ph ph-users-three'"
                    :title="$tab['emptyState']['title'] ?? '还没有内容'"
                    :description="$tab['emptyState']['description'] ?? '当前标签下还没有可显示的数据。'"
                    container-class="flex min-h-[26rem] w-full flex-col items-center justify-center px-6 py-12 text-center"
                    icon-shell-class="mx-auto flex items-center justify-center text-[3rem] text-gray-300"
                    title-class="mt-4 text-xl font-semibold tracking-tight text-gray-950 sm:text-2xl"
                    description-class="mt-3 mx-auto w-full max-w-xl text-sm leading-7 text-gray-500 sm:text-base"
                  />
                </div>
              @endif
            </div>
