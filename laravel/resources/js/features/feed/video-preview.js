...
function createPlyrHoverPreviewController(container, video) {
  const playbackSession = createPlaybackSession(video);
  let isDestroyed = false;
  let player = null;
  ...
  const setPreviewing = (isPreviewing) => {
  ...
  const ensurePlayer = () => {
    ...
    player.on("playing", handlePlayerPlay);
    player.on("pause", handlePlayerPause);
    player.on("ended", handlePlayerPause);
    ...
    playerReadyPromise = new Promise((resolve) => {
      ...
      const readyFallbackTimeout = window.setTimeout(() => {
        player.off?.("ready", handleReady);
        settle();
      }, 1500);

      player.on("ready", handleReady);
      scheduleTask(syncPlayerContainer);
    }).finally(() => {
      playerReadyPromise = player ? Promise.resolve(player) : null;
    });
  ...
  const playPreview = async () => {
    const ensuredPlayer = await ensurePlayer();
    await playbackSession.prepare();
    ...
    if (ensuredPlayer?.play) {
      ensuredPlayer.play().catch(() => {});
      return;
    }
  ...
  const handlePlaybackError = () => {
    ...
    void playbackSession.fallback().then(() => {
      if (isDestroyed || activeHoverPreviewController !== controllerApi) {
        return;
      }
  ...
  const controllerApi = {
    ...
    destroy() {
      ...
      player.off?.("playing", handlePlayerPlay);
      player.off?.("pause", handlePlayerPause);
      player.off?.("ended", handlePlayerPause);
      player.destroy();
      player = null;
      playerReady = false;
...
export function installHoverVideoPreview(container, video) {
  ...
  if (typeof window.Plyr === "function") {
    return createPlyrHoverPreviewController(container, video);
  }
